...
def update_all_messages():
    ...
    msg += f"<b>🖥️ 𝘾𝙋𝙐:</b> <code>{psutil.cpu_percent()}%</code>" \
           f" <b>🚀 𝙍𝘼𝙈:</b> <code>{psutil.virtual_memory().percent}%</code>" \
           f" <b>📦 𝘿𝙄𝙎𝙆:</b> <code>{psutil.disk_usage('/').percent}%</code>"
    with download_dict_lock:
        ...
        msg += f"\n<b>𝙁𝙍𝙀𝙀:</b> <code>{free}</code> | <b>𝙐𝙋𝙏𝙄𝙈𝙀:</b> <code>{currentTime}</code>\n<b>𝘿𝙇:</b> <code>{dlspeed}/s</code> 🔻 | <b>𝙐𝙇:</b> <code>{ulspeed}/s</code> 🔺\n"
    with status_reply_dict_lock:
...
def sendStatusMessage(msg, bot):
    ...
    progress += f"<b>🖥️ 𝘾𝙋𝙐:</b> <code>{psutil.cpu_percent()}%</code>" \
           f" <b>🚀 𝙍𝘼𝙈:</b> <code>{psutil.virtual_memory().percent}%</code>" \
           f" <b>📦 𝘿𝙄𝙎𝙆:</b> <code>{psutil.disk_usage('/').percent}%</code>"
    with download_dict_lock:
        dlspeed_bytes = 0
